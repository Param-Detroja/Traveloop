...
echo "Downloading All City Images (Parallel 4K)..."
...
download_city() {
  ...
  curl -s -o "public/images/cities/$filename.jpg" "https://image.pollinations.ai/prompt/$encoded_prompt?width=1024&height=576&nologo=true"
}
...
for city_info in "${cities[@]}"; do
  filename="${city_info%%:*}"
  prompt="${city_info#*:}"
  download_city "$filename" "$prompt" &
done

wait
echo "Done! All city images downloaded in parallel."
